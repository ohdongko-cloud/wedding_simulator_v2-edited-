...
export default function ExtraSection({ onChange }: ExtraSectionProps) {
  ...
  const updatePrice = (id: string, val: string) => {
    ...
    setItems(prev => prev.map(i => (i.id === id ? { ...i, price: formatted } : i)));
  };
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-blue-100 overflow-hidden mb-4">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-white/70 text-xs ml-1">({items.length}개 항목)</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-white font-bold text-lg">{total.toLocaleString('ko-KR')} 만원</span>
      ...
      {collapsed && (
        <div className="px-5 py-2 bg-gray-50 border-t border-blue-100 flex items-center justify-between">
          ...
          <span className="text-sm font-bold text-gray-600">합계: {total.toLocaleString('ko-KR')} 만원</span>
        </div>
      )}
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    ...
                    <th className="px-4 py-2 text-center text-xs font-semibold text-gray-400">금액(만원)</th>
                    <th className="px-4 py-2 text-center text-xs font-semibold text-gray-400 w-12">삭제</th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((item, idx) => (
                ...
                <tfoot>
                  <tr className="border-t-2 border-blue-100">
                    <td className="px-4 py-3 text-xs text-gray-400">{items.length}개 항목</td>
                    <td colSpan={2} className="px-4 py-3 text-right">
                      <span className="font-bold text-gray-700">소계: </span>
                      <span className="font-black text-gray-800 text-base">{total.toLocaleString()} 만원</span>
                    </td>
                  </tr>
                </tfoot>
              </table>
