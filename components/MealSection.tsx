...
export default function MealSection({ onChange }: MealSectionProps) {
  ...
  const [selectedPrice, setSelectedPrice] = useState<number | null>(77000);
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-rose-100 overflow-hidden mb-4">
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <label className="text-sm font-semibold text-gray-600 min-w-[80px]">인원수</label>
          <div className="flex items-center gap-2">
            <input
              type="number"
              min={200}
              value={headcount}
              onChange={(e) => setHeadcount(Math.max(200, parseInt(e.target.value) || 200))}
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-3">
          <label className="text-sm font-semibold text-gray-600 min-w-[80px] pt-2">1인 가격</label>
        ...
        <div className="bg-rose-50 rounded-xl px-4 py-3 flex flex-wrap items-center gap-2 text-sm">
          ...
          <span className="font-semibold text-gray-700">{headcount.toLocaleString()}명</span>
          <span className="text-gray-400">×</span>
          <span className="font-semibold text-gray-700">{unitPrice.toLocaleString()}원</span>
          <span className="text-gray-400">=</span>
          <span className="font-bold text-rose-600">{totalMan.toLocaleString('ko-KR')} 만원</span>
        </div>
      </div>
    </div>
  );
