...
function calcTotal(items: CheckItem[]): number {
  return items
    ...
    .reduce((sum, i) => {
      ...
      if (i.isCustomRow) {
        return (
          sum + (parseInt((i.avgPriceCustom || "").replace(/,/g, "")) || 0)
        );
      }
...
function useItemState(initial: CheckItem[]) {
  ...
  const customChange = (id: string, val: string) =>
    setItems((prev) =>
      prev.map((i) => (i.id === id ? { ...i, customPrice: val } : i))
    );

  const addRow = (item: CheckItem) => setItems((prev) => [...prev, item]);
  ...
  const avgPriceChange = (id: string, val: string) =>
    setItems((prev) =>
      prev.map((i) => (i.id === id ? { ...i, avgPriceCustom: val } : i))
    );
...
export default function WeddingSimulator() {
  ...
  const extraState = useItemState(EXTRA_ITEMS);
  ...
  const extraTotal = calcTotal(extraState.items);
        ...
        {activeTab === "wedding" ? (
          <>
            ...
            <ItemTable
              title="기타"
              emoji="🧾"
              items={extraState.items}
              colorClass="from-gray-500 to-slate-600"
              borderColor="border-gray-100"
              accentColor="gray"
              totalMan={extraTotal}
              onToggle={extraState.toggle}
              onCustomChange={extraState.customChange}
              onAddRow={extraState.addRow}
              onRemoveRow={extraState.removeRow}
              onAvgPriceChange={extraState.avgPriceChange}
              onNameChange={extraState.nameChange}
            />
            ...
            <div className="mt-6 bg-gradient-to-br from-rose-500 to-pink-600 rounded-3xl p-6 text-white shadow-xl text-center">
              ...
              <p className="text-4xl font-black tracking-tight mb-1">
                ...
                <span className="text-2xl font-bold text-rose-200 ml-1">
                  만원
                </span>
              </p>
